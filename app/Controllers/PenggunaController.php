...
class PenggunaController extends BaseController
{
	public function index(){
		return view('halaman/pengguna/table', [
			'AY' => (new Pengguna12200603())->get()->getResult(),
			'error' => $this->session->getFlashdata('error')
		]);
	}

	public function form($data = []){
		return view('halaman/pengguna/from', [
	...
	public function simpan(){
		...
		if( $this->validasi() ){
			$r = (new Pengguna12200603())->insert($data);
			if($r = false){
	...
	public function edit($id){
		$data = (new Pengguna12200603())->where('id', $id)->first();

		if($data == null){
			return redirect()->to('/pengguna-list')->with('error','Pengguna tidak ditemuakan');
		}else{
	...
	public function patch(){
		...
		if( $this ->validasiPatch() ){
			$r = (new Pengguna12200603())->update($id, $data);
			if($r == true){
				return redirect()->to('/pengguna-list');
			}else{
	...
	public function delete(){
		$id = $this->request->getPost('id');
		$r = (new Pengguna12200603())->delete($id);

		$rd = redireck()->to('/pengguna-list');
		if($r == false){
